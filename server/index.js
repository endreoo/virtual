...
app.post('/api/cards/manual-payment', async (req, res) => {
  console.log('[API] Processing manual payment request:', req.body);
  try {
    const { 
      reservation_id,
      amount_usd,
    ...
    // Get the hotel_id from expedia_reservations
    console.log('[API] Fetching hotel_id for reservation:', reservation_id);
    const [hotelRows] = await pool.execute(
      'SELECT hotel_id FROM expedia_reservations WHERE id = ?',
      [reservation_id]
    ...
    // Update reservation status
    const updateQuery = 'UPDATE expedia_reservations SET status = ? WHERE id = ?';
    const updateValues = ['Manual Payment', reservation_id];
    ...
    console.error('[API] Error processing manual payment:', error);
    res.status(500).json({ 
      status: 'error',
      message: error.message || 'Failed to process manual payment request'
    });
  }
});
